...
2. **Instruction Drift:** Initial instructions (e.g. "Do not write code in chat", "Always check tests", "Use TypeScript only") vanish from the active context window over long sessions.
3. **Hallucinated State:** An agent with degraded context begins guessing what files exist or what architecture was agreed upon earlier.
...
```
┌─────────────────────────────────────────────────────────────┐
│ 1. PERSISTENT INSTRUCTIONS (Tier 1: SKILL.md, AGENTS.md)    │
│    Reloaded on every /chief or /chief+ call                 │
├─────────────────────────────────────────────────────────────┤
│ 2. PERSISTENT MEMORY (Tier 2: memory/manager/*.md)          │
...
Instead of stuffing thousands of tokens of test logs, file listings, and diffs into the chat, Manager compresses worker outputs into 4-line summaries and writes key takeaways directly to [`memory/manager/project_state.md`](file:///home/ishaan/Work/ai-chief/memory/manager/project_state.md).
...
You can start a project in Google Antigravity, switch to Claude Code in the terminal, and later inspect it in Cursor. Each tool simply reads [`SKILL.md`](file:///home/ishaan/Work/ai-chief/SKILL.md) and [`AGENTS.md`](file:///home/ishaan/Work/ai-chief/AGENTS.md) and picks up right where the other left off.
...
Turn 50:  Agent is doing well.
Turn 200: Context limit hit. Platform silently compresses early turns.
Turn 350: Initial rules disappear. Agent begins hallucinating patterns.
Turn 500: Total failure. Agent overwrites core files and dumps raw logs.
```

### ✅ The Resilient AI-Chief Workflow
```text
Task 1:   User: "/chief add user database models"
          -> AI reads SKILL.md & memory/ -> Chief mode -> Manager plans -> Worker codes -> Memory synced.

Task 50:  User: "/chief add password hashing"
          -> AI re-verifies memory/manager/decisions.md -> ADRs respected -> Output compressed.
...
2. **Use `/chief+` Whenever Behavior Drifts:**
   If the AI ever writes unsolicited code directly in chat, dumps terminal logs, or forgets a previous decision, simply issue:
   ```text
   /chief+ [task]
   ```
